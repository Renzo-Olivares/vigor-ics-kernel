/* linux/arch/arm/mach-msm/board-vigor-wifi.c
*/
...
int vigor_wifi_reset(int on);
int vigor_wifi_set_carddetect(int on);
int vigor_wifi_get_mac_addr(unsigned char *buf);
...
static void *vigor_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init vigor_init_wifi_mem(void)
{
	int i;
...
static struct resource vigor_wifi_resources[] = {
	[0] = {
...
static unsigned vigor_wifi_update_nvs(char *str)
{
...
#else
	vigor_wifi_update_nvs("sd_oobonly=1\n");
#endif
	vigor_wifi_update_nvs("btc_params80=0\n");
	vigor_wifi_update_nvs("btc_params6=30\n");
	vigor_init_wifi_mem();
	ret = platform_device_register(&vigor_wifi_device);
	return ret;
}
