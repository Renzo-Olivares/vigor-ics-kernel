...
// void __init vigor_init_panel(struct resource [], int);
...
#ifdef CONFIG_PERFLOCK
static unsigned vigor_perf_acpu_table_1188k[] = {
	384000000,
	756000000,
	1188000000,
};
static unsigned vigor_perf_acpu_table_1512k[] = {
	540000000,
	1026000000,
	1512000000,
};
...
static int vigor_phy_init_seq[] = { 0x06, 0x36, 0x0C, 0x31, 0x31, 0x32, 0x1, 0x0E, 0x1, 0x11, -1 };
...
static uint32_t usb_ID_PIN_input_table[] = {
	GPIO_CFG(VIGOR_GPIO_USB_ID, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static uint32_t usb_ID_PIN_ouput_table[] = {
	GPIO_CFG(VIGOR_GPIO_USB_ID, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static void config_vigor_usb_id_gpios(bool output)
{
	if (output) {
		...
		gpio_set_value(VIGOR_GPIO_USB_ID, 1);
		printk(KERN_INFO "%s %d output high\n",  __func__, VIGOR_GPIO_USB_ID);
	} else {
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.mhl_reset_gpio		= VIGOR_GPIO_MHL_RST_N,
...
static struct camera_led_est msm_camera_flash_led_table[] = {
	{
		.enable = 0,
...
static struct camera_led_info msm_camera_flash_led_info = {
	.enable = 1,
	.low_limit_led_state = FL_MODE_FLASH_LEVEL1,
	.max_led_current_ma = 800,
	.num_led_est_table = ARRAY_SIZE(msm_camera_flash_led_table),
};
...
static struct camera_flash_info msm_camera_sensor_flash_info = {
	.led_info = &msm_camera_flash_led_info,
	.led_est_table = msm_camera_flash_led_table,
};
/* HTC end */
...
static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(VIGOR_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GENERAL_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
*/
...
/*
	...
	RPM_LDO(PM8058_L12, 0, 1, 0, 3200000, 3200000, LDO150HMIN),
...
*/
        ...
        RPM_LDO(PM8058_L0,  0, 0, 0, 1200000, 1200000, LDO300HMIN),
        RPM_LDO(PM8058_L1,  0, 1, 0, 1200000, 1200000, LDO150HMIN),
        RPM_LDO(PM8058_L2,  0, 1, 0, 1200000, 1200000, LDO150HMIN),
        RPM_LDO(PM8058_L3,  0, 1, 0, 1200000, 1200000, LDO150HMIN),
        RPM_LDO(PM8058_L4,  0, 1, 0, 2850000, 2850000, LDO50HMIN),
        RPM_LDO(PM8058_L5,  0, 0, 0, 2850000, 2850000, LDO300HMIN),
        RPM_LDO(PM8058_L6,  0, 0, 0, 3000000, 3600000, LDO50HMIN),
        RPM_LDO(PM8058_L7,  0, 0, 0, 1800000, 1800000, LDO50HMIN),
        ...
        RPM_LDO(PM8058_L15, 0, 1, 0, 2800000, 2800000, LDO300HMIN),
        RPM_LDO(PM8058_L16, 1, 1, 1, 1800000, 1800000, LDO300HMIN),
        RPM_LDO(PM8058_L17, 0, 1, 0, 1200000, 1290000, LDO150HMIN),
        RPM_LDO(PM8058_L18, 0, 1, 1, 2200000, 2200000, LDO150HMIN),
        RPM_LDO(PM8058_L19, 0, 1, 0, 1800000, 1800000, LDO150HMIN),
        RPM_LDO(PM8058_L20, 0, 1, 0, 1200000, 1200000, LDO150HMIN),
        RPM_LDO(PM8058_L21, 1, 0, 0, 1200000, 1200000, LDO150HMIN),
        RPM_LDO(PM8058_L22, 0, 0, 0, 1200000, 1200000, LDO300HMIN),
        RPM_LDO(PM8058_L23, 0, 0, 0, 1200000, 1200000, LDO300HMIN),
        RPM_LDO(PM8058_L24, 0, 1, 0, 1200000, 1200000, LDO150HMIN),
...
//	RPM_LDO(PM8901_L4,  0, 1, 0, 1800000, 1800000, LDO300HMIN),
	RPM_LDO(PM8901_L4,  0, 1, 0, 1200000, 1200000, LDO150HMIN),
...
static uint32_t mdm2ap_gpio_table[] = {
	...
	GPIO_CFG(VIGOR_AP2MDM_WAKEUP, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
};
...
#ifdef CONFIG_MMC_MSM_SDC1_SUPPORT
static unsigned int vigor_sdc1_slot_type = MMC_TYPE_MMC;
...
#endif
	.slot_type		= &vigor_sdc1_slot_type,
	.msmsdcc_fmin	= 400000,
...
#ifdef CONFIG_MMC_MSM_SDC3_SUPPORT
static unsigned int vigor_sdc3_slot_type = MMC_TYPE_SD;
...
#endif
	.slot_type		= &vigor_sdc3_slot_type,
	.msmsdcc_fmin	= 144000,
	.msmsdcc_fmid	= 24000000,
	.msmsdcc_fmax	= 48000000,
	.nonremovable	= 0,
...
static void vigor_panel_power(int on)
{
	...
	if (!init) {
		...
		/* LCM Reset */
		rc = gpio_request(VIGOR_GPIO_LCM_RST_N, "LCM_RST_N");
		if (rc) {
			printk(KERN_ERR "%s:LCM gpio %d request"
				"failed\n", __func__,  VIGOR_GPIO_LCM_RST_N);
			return;
		}

		// gpio_direction_output(VIGOR_GPIO_LCM_RST_N, 0);
		init = 1;
	}
	...
	if (on) {
		...
		mdelay(10);
		gpio_set_value(VIGOR_GPIO_LCM_RST_N, 1);
		mdelay(1);
		gpio_set_value(VIGOR_GPIO_LCM_RST_N, 0);
		mdelay(1);
		gpio_set_value(VIGOR_GPIO_LCM_RST_N, 1);
		mdelay(20);
	} else {
...
static uint32_t msm_spi_gpio[] = {
	GPIO_CFG(VIGOR_SPI_DO,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_SPI_DI,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_SPI_CS,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_SPI_CLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
};
...
static struct msm_board_data vigor_board_data __initdata = {
...
#ifdef CONFIG_PERFLOCK
	if (vigor_perf_acpu_table_1188k[PERF_LOCK_HIGHEST] == get_max_cpu_freq() * 1000) {
		vigor_perflock_data.perf_acpu_table = vigor_perf_acpu_table_1188k;
		vigor_perflock_data.table_size = ARRAY_SIZE(vigor_perf_acpu_table_1188k);
	} else {
		vigor_perflock_data.perf_acpu_table = vigor_perf_acpu_table_1512k;
		vigor_perflock_data.table_size = ARRAY_SIZE(vigor_perf_acpu_table_1512k);
	}
				 ...
				 char **cmdline, struct meminfo *mi)
{
	...
	printk(KERN_INFO "vigor_fixup:skuid=0x%x\n", skuid);
}
